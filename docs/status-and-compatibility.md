...
**Status:** Target resolved for naming; source and client-registration search required before release. Two client-visible names deliberately do NOT follow the plugin id and must not be "fixed" by that search: the Claude Code MCP server name stays `governor`, with tool prefixes `mcp__governor__*`, because the client-visible name is the tool prefix and renaming it would break agent sessions for no semantic gain; and five shipped tool names keep their exact spellings when the provider publishes them, through a closed host-held grandfather table that names the one owner id allowed to publish each.
