...
- **The five governance tools keep their exact names** — `governance_pending_review`, `governance_revisions`, `governance_submit_revision`, `governance_mandate_draft`, `governance_mandates` — through a closed grandfather table in the host (`GRANDFATHERED_TOOL_NAMES`). Every other satellite paid the `<plugin id>_<bare name>` rename tax; these five were already on the wire.
