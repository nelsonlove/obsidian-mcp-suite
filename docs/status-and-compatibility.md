...
Installed release: **0.18.2 (2026-08-24)**, still a single plugin. The release status must name the highest completed gate. Code presence is not deployment, migration closure, portability evidence, or Community readiness.

- **The host/provider split (S3c) is BUILT, NOT YET CUT OVER on the operator's vault.** 0.19.0 is built and unreleased: two Obsidian plugins where 0.18.2 shipped one — the MCP host (`packages/host`, plugin id `vault-mcp`, "Vault MCP") and the governance provider (`packages/governor`, plugin id `governor`, "Governor"), both at 0.19.0. The installed vault still runs the single 0.18.2 artifact under the id `governor`, so nothing in this section's shipped claims changes yet. What the split changes on the day it lands, and the order the changes must land in, is [the S3c migration plan](s3c-migration-plan.md); this section supersedes it as the owner of shipped truth the moment the cutover runs. Two facts operators need before that day: the Claude Code MCP server name moves back to `vault-mcp` with the plugin id (Nelson's ruling, 2026-09-09 — tool prefixes become `mcp__vault-mcp__*`, and any `mcp__governor__*` permission entry needs re-adding under the new prefix, while the old registration keeps working until it is removed), and the provider keeps `.obsidian/plugins/governor/` with all of `governance/` in place, so no authority state moves.
