...
| Effect | Vault-relative folder prefixes, one per line, that observation capture never retains and the conformance rail never walks — the areas that must not leave the vault. Committed when the field loses focus. An entry covers its own folder and nothing that merely continues its name: `80-89` covers `80-89 Divorce/` and `80-89/` but not `80-891/` or `80-89-archive/`; end an entry with `/` to mean exactly that folder |
