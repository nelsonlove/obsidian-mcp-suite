# Vault MCP Host — Authoritative Tool Inventory

(Host plugin id `vault-mcp`. It was `vault-mcp` before 0.12.0, `governor` from
0.12.0 through 0.18.2, and `vault-mcp` again from 0.19.0, when the S3c split
gave `governor` to the governance provider that now ships as its own plugin —
`packages/governor`. This file inventories the HOST's surface; the provider's
five published tools have their own inventory at
`packages/governor/TOOL-INVENTORY.md`.)

Source of record for every tool the host plugin's MCP server registers.
Generated by reading `packages/host/src/mcp/server.ts` and all `tools-*.ts`
files.
...
governance provider at S3c, along with the one `obsidian_*` tool that went with
...
the pending-review listing (since S3c the provider's, so unbackticked here —
the lock scrapes backticked names), `obsidian_plugin_info`,
...
the snippet tools);
the same plugin set today registers 17 + 42 + 6 + 6 = **71** (the vocab
module's four left the host at S7, along with health's two and bases' two; the
mutating tier's eighteen were default-disabled and never in this count; and one
always-live tool left for the governance provider at S3c).
...
### Section 2's governance rows — MOVED to the governance provider (S3c)

Three always-on rows stood here until the host/governor split: the pending-review listing (`tools-pending-review.ts`, kernel B3b) and the two revision tools (`tools-governance-revision.ts` — `registerGovernanceRevisionTool` / `registerGovernanceRevisionsListTool`, #101/#221). Two mandate tools sat beside them in source and were never written up here at all. All five now live in `packages/governor/src/tools/` and are PUBLISHED into this host through `vault-mcp-api`, exactly the way a third-party plugin publishes.

**Their names did not change**, and that is a deliberate, closed exception rather than a pattern. The ordinary publishing rule is `<sanitized owner id>_<bare name>`, and the host REFUSES any published name beginning `obsidian_` (`external-tools.ts`'s F1 check) — the rule that forced the seven `obsidian_jd_*` renames. Under it the revision tools would have become `governor_governance_revisions` and the pending-review tool could not have been published at all. So `external-tools.ts` carries `GRANDFATHERED_TOOL_NAMES`: a checked-in map from each of the five spellings to the SINGLE owner id (`governor`) permitted to publish it. Nothing else reaches the reserved namespace, no publisher-controlled flag exists that could claim one, and the table is closed — a new tool takes the ordinary namespaced form. It exists only for continuity of names that were already on the wire when the split happened.

The names are deliberately not backticked in this file: the source↔doc lock scrapes backticked `obsidian_*` names and would read a departed one as a name this host ought to register. `packages/governor/TOOL-INVENTORY.md` carries all five in full, with their published spellings, their read-only hints, and their allowlist posture under this host's gate — which got STRICTER in the move, the same way it did at every previous extraction: an external tool's read-only claim is distrusted unless the operator lists `governor` in `trustedReadOnlyPlugins`, and under an active path allowlist the F3 gate refuses any external tool whose arguments carry no recognized path key.
...
| `obsidian_plugin_uninstall` | W | `plugin:uninstall` | DANGEROUS + `destructiveHint: true` — same gate; refuses to uninstall the host plugin itself, and refuses a registered governance provider's id for the same reason (§5 condition 6: an agent doing cleanup must not be one call away from switching off the thing that reviews it) |
...
- 17 fs-expressible ✓
- 22 always-live at the time ✓ (now 42 + 6 module-mounted — see Sections 2/2b)
...
same plugin set the current surface registers 70 (see the count summary); the
...
| `packages/host/src/mcp/tools-locks.ts` | `registerLockTools` | 4 always-live |
| `packages/host/src/mcp/tools-uid.ts` | `registerUidTools` | 1 always-live |
| `packages/host/src/mcp/tools-links.ts` | `registerLinkTools` | 1 always-live |
| `packages/host/src/mcp/tools-conformance-debt.ts` | `registerConformanceDebtTools` + `registerConformanceDebtRenderTool` | 2 always-live |
| `packages/host/src/mcp/tools-write-notes.ts` | `registerWriteNotesTool` | 1 always-live |
| `packages/host/src/mcp/tools-scheme.ts` | `registerSchemeTools` (via `modules-mount.ts`) | 6 module-mounted |
| `packages/host/src/mcp/tools-snippets.ts` | `registerSnippetTools` | 4 always-live |
| `packages/host/src/mcp/tools-integrations.ts` | `registerIntegrationTools` | up to 6 conditional |
| `packages/host/src/mcp/tools-import.ts` | `registerImportTools` | 1 conditional (Importer plugin, version-gated) |
| `packages/host/src/mcp/tools-cli-dedicated.ts` | `registerCliDedicatedTools` | 5 conditional (CLI binary) |
| `packages/host/src/mcp/tools-cli.ts` | `registerCliTools` | 1 conditional (CLI binary + "Raw CLI proxy" setting, default off) |
| `packages/host/src/mcp/tools-code-mode.ts` | `registerCodeModeTools` | 3 (alternative surface, uncounted) |
| `packages/host/src/mcp/external-tools.ts` | `ExternalToolRegistry` (+ `GRANDFATHERED_TOOL_NAMES`) | every tool any other plugin publishes, the governance provider's five included — inventoried as a mechanism, never as rows here |
