...
| `snapshot.ts` | Headless disk source: parses frontmatter/bodies into the listings packs accept (reuses `@vault-mcp/core` parsing; accepts an `excludedRoots` exclusion list — wired from the invocation via `--exclude=`/`GOVERNOR_EXCLUDED_ROOTS` (legacy `ASSENT_EXCLUDED_ROOTS`), with `excludedRootRefusal` refusing a run whose exclusions would strand accepted baseline keys). Also the territory chokepoint (#157): `buildSnapshot` refuses a root with no declared boundary, a root resolving outside it, and a root or symlink resolving into one of the operator's guarded territories (`--territory=`/`GOVERNOR_TERRITORIES` on the CLI, the host's `guardedTerritories` setting inside Obsidian; NO built-in list since #397). A listed territory met INSIDE the root is skipped and reported, not refused (#398), and a baseline key under a skipped folder refuses the run rather than reading CLEARED — decided over resolved real paths (`path-identity.ts`), before any read. |
...
- A **dead convention path is loud** (#298): every path-valued key of `DEFAULT_VAULT_CONVENTIONS` / `GOVERNOR_VAULT_CONVENTIONS` is checked against the walk before the legacy packs run; a key that names nothing becomes a `conformance_engine / dead_convention` finding (NEW, so the run fails), every pack that reads it (`registriesRoot` feeds two: `drift_audit` and `conformance_check`) is registered but not run and not counted as covered, and the report prints `DEAD CONVENTION: <key> = <path>`. An engine finding is never rebaseline text — it describes the run, not the vault — so a dead convention is NEW on every run and cannot be silenced through `--rebaseline`; when the baseline names an unmeasured pack, the coverage refusal says which convention is dead and how to set it. Nothing guesses a replacement path — where a convention should point is a vault filing question, answered through `GOVERNOR_VAULT_CONVENTIONS`. As of 2026-09-24 every shipped default but `systemRoot` is dead on this vault (two renumberings since they were written), so the legacy packs do not measure here until the operator re-points them or retires them.
- Both in-app adapters (`obsidian_conformance_debt`, the drift pane) apply **`coverageRefusal`** after their run (#294), the same exported function `runCli` uses: a pack the baseline describes that did not run — threw, or its convention path is dead — refuses rather than reporting its accepted keys CLEARED.
