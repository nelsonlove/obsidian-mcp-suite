...
| Health, survey, Fileclass inspection, and provenance inspection/staleness | Public optional | Disabled until configured or supported; regeneration remains private. Only survey is still one of this plugin's modules. The health scan ships as the separate `vault-health` satellite plugin since S7, whose tools publish as `vault_health_scan` / `vault_health_lint`; Fileclass inspection and provenance inspection followed with the mutating tier, as the separate `vault-fileclass` and `vault-provenance` satellite plugins, publishing `vault_fileclass_*` and `vault_provenance_*`. Both are blocked wholesale under a path allowlist rather than partially scoped — see their package READMEs |
